...
export const contractScanExtractionSchema = z.object({
  summary: z.object({
    ...
    negotiationPoints: z.array(z.string().min(1).max(300)).min(1).max(8).default([]),
    priorityActions: z.array(z.string().min(1).max(300)).min(1).max(8).default([]),
    clausesNeedingLegalReview: z.array(z.string().min(1).max(300)).min(0).max(8).default([]),
  }),
